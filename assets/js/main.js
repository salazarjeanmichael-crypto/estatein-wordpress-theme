...
(function () {
	...
	function initDismiss() {
		...
		Array.prototype.forEach.call(buttons, function (button) {
			...
			// A dismissal from an earlier page view is applied by the inline
			// head script, before the first paint. Repeating it here would
			// only hide the bar after it had already been shown.
	...
	function initCarousel(root) {
		...
		function render() {
			...
			items.forEach(function (item, i) {
				...
				Array.prototype.forEach.call(item.querySelectorAll("a, button, input"), function (el) {
					if (visible) {
	...
	// Offset between siblings, and how many of them still earn one. Past the
	// cap a wide row would still be arriving well after the scroll that
	// brought it in, which reads as lag rather than sequence.
	var REVEAL_STEP = 60;
	var REVEAL_CAP = 4;

	/**
	 * Document order, for sorting a batch of elements.
	 */
	function revealOrder(a, b) {
	...
	/**
	 * Drop everything the entrance added, once it is over.
	 */
	function revealSettle(el) {
	...
	function initReveal() {
		...
		var stagger = window.matchMedia("(min-width: 701px)");

		var io = new IntersectionObserver(
			function (entries) {
				// Counted per callback, so an element's delay depends on what it
				// is arriving alongside rather than on its DOM index. A row
				// scrolled to on its own leads at 0ms instead of sitting out a
				// delay earned by siblings that came in long before it.
				var batch = [];
				var groups = [];
				var counts = [];
				...
				// Entry order is not specified, and the stagger has to run down
				// the page to read as one movement.
				batch.sort(revealOrder);

				batch.forEach(function (el) {
					if (!el.hasAttribute("data-reveal-fixed")) {
						var index = 0;

						// Stagger siblings, capped so a long grid does not crawl in.
						// Skipped on a phone, where siblings are stacked and a
						// delayed sibling only reads as an item arriving late.
						if (stagger.matches) {
							var at = groups.indexOf(el.parentNode);

							if (at === -1) {
								at = groups.push(el.parentNode) - 1;
								counts[at] = 0;
							}
						...
						// Written even at 0ms, because this property inherits: left
						// unset it would fall through from a staggered ancestor,
						// then shift when that ancestor cleared its own.
						el.style.setProperty("--reveal-delay", Math.min(index, REVEAL_CAP) * REVEAL_STEP + "ms");
					}

					// Dropped again in revealSettle; held on every target it would
					// leave the page carrying layers with nothing to composite.
					el.style.setProperty("will-change", "opacity, transform");
					el.classList.add("is-visible");
				});
			},
			{
				// A flat trigger line just inside the fold. A threshold is
				// measured against the element, so a short card and a
				// viewport-tall panel would start at unrelated scroll positions.
				threshold: 0,
		...
		Array.prototype.forEach.call(document.querySelectorAll(REVEAL), function (el) {
			...
			// Read from the cascade, not the inline style, so a delay the
			// stylesheet set survives. The hero is choreographed there.
			if (window.getComputedStyle(el).getPropertyValue("--reveal-delay").trim()) {
				el.setAttribute("data-reveal-fixed", "");
			}

			el.addEventListener("animationend", function (e) {
				// animationend bubbles and several of these nest, so without
				// this a child finishing would settle its ancestor and cut
				// that ancestor's own animation off part-way.
				if (e.target === el) {
					revealSettle(el);
				}
			});

			// Reached if the animation is replaced rather than finished, such
			// as on a resize across the breakpoint. Better a settled element
			// than one stranded at opacity 0.
